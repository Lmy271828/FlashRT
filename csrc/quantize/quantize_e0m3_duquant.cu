...
//
//  Thread mapping (both kernels): one warp per (row, 64-block). Each lane
//  owns output channels c = lane and c = lane + 32. The 64 gathered inputs
//  live one-per-lane in registers (two per lane) and are broadcast with
//  __shfl_sync, so the rotation matrix is read fully coalesced
//  (R[d][lane], R[d][lane+32] across the warp) and the input row is read
...
__global__ void kernel_quantize_e0m3_duquant_sfa(
    ...
    if (row >= M || warp >= nb) return;

    const __nv_bfloat16* srow = src + static_cast<size_t>(row) * K;
    const __nv_bfloat16* rblk = rot + static_cast<size_t>(warp) * 4096;
    ...
    for (int d = 0; d < 64; ++d) {
        ...
        acc0 = fmaf(v, __bfloat162float(rblk[(d << 6) + lane]), acc0);
        acc1 = fmaf(v, __bfloat162float(rblk[(d << 6) + lane + 32]), acc1);
    }
    ...
    if (act_scale != nullptr) {
        const float s0 = __bfloat162float(__float2bfloat16(act_scale[base + lane]));
        const float s1 = __bfloat162float(__float2bfloat16(act_scale[base + lane + 32]));
    ...
    // Every lane now holds its group's amax (identical IEEE-exact results
    // across the 16 lanes, so no extra broadcast is needed).
    const float desired0 = e0m3_scale_from_amax(a0);
    const float desired1 = e0m3_scale_from_amax(a1);
    const float scale_dq0 = static_cast<float>(quantize_ue4m3_e0m3(desired0));
    const float scale_dq1 = static_cast<float>(quantize_ue4m3_e0m3(desired1));
    ...
    if (lane == 0 || lane == 16) {
        ...
        const int blk16 = warp << 2;  // first 16-block of this 64-block
...
__global__ void kernel_duquant_rotate_out(
    ...
    const __half* rblk = rot + static_cast<size_t>(warp) * 4096;
    const int base = warp << 6;
    ...
    for (int d = 0; d < 64; ++d) {
        const float v = __shfl_sync(0xffffffffu, d < 32 ? in0 : in1, d & 31);
        acc0 = fmaf(v, __half2float(rblk[(d << 6) + lane]), acc0);
        acc1 = fmaf(v, __half2float(rblk[(d << 6) + lane + 32]), acc1);
    }

    // Replicate: bmm output rounded to fp16, then cast to bf16, then bias
    // add in fp32 math rounded to bf16.
...
int quantize_e0m3_duquant_sfa_bf16(
  ...
  dim3 grid((nb + threads / 32 - 1) / (threads / 32), M);
